...
- The image (`Images/WindupBird.jpg`, ~158 KB JPEG) is imported as a Vite module asset — not served from `public/` — so it is fingerprinted and bundled with the app.
...
- Subtitle: `title="The wind-up bird..."` — mysterious, not literal. `cursor="pointer"` hints interactivity without advertising the secret.

## Alternatives Considered

- **Floating `Box` overlay** instead of `Modal`: rejected — Chakra `Modal` handles focus trap, escape-to-close, and overlay backdrop out of the box with less custom code.
- **Serving image from `public/`**: skipped — no `public/` directory exists in this project; module import is the canonical Vite approach and produces the same result.
